...
public class LoopingXMLDocument {
    ...
    private String templateRegionName = "REGION";
    private String templateRegionID = "0";
    private ActivityDetails[] activities = new ActivityDetails[545];
    private CadreDetails[] cadresDetails;
    private int activityLength = 0;
                                            ...
                                            if( elementChilds.item(l).getNodeType() == Node.ELEMENT_NODE ){
                                                if( elementChilds.item(l).getAttributes().getNamedItem("key").getTextContent().compareToIgnoreCase("spec_group") == 0 )
                                                {
                                                    if( elementChilds.item(l).getAttributes().getNamedItem("value").getTextContent().compareToIgnoreCase(cadreID) == 0 ){
                                                        ...
                                                        getActivities()[activityLength] = new ActivityDetails(name, id, specGroupValue);//insert activities sequencially because of activityLength   
                                                        activityLength++;
...
//                System.out.println(environmentChilds.item(i).getAttributes().getNamedItem("id").getTextContent());
    ...
    public String searchActivityID(String atcName, String cadreID){
        ...
        for( int i=0;i< activities.length; i++){
            ...
            if( activities[i].getName().trim().compareToIgnoreCase(atcName.trim()) == 0) 
                    if(activities[i].getSpecGroupValue().compareToIgnoreCase(cadreID) == 0)
     ...
     */
    public ActivityDetails[] getActivities() {
        return activities;
    }
     ...
     */
    public void setActivities(ActivityDetails[] activities) {
